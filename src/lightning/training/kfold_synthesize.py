...
from src.dataset.synthesize import synthesize_data
...
def train(hparams):
    ...
    tensor_dataset = TensorDataset(torch.tensor(dataset.X.values, dtype=torch.float32), torch.tensor(dataset.y.values, dtype=torch.float32))

    project_path = project.set_project(hparams.project_name)
    experiment_name = hparams.experiment_name
    run_name = hparams.run_name

    if folds > len(dataset.X):
    ...
    if folds == len(dataset.X) or hparams.loocv ==True:
    ...
    for epoch in range(1, hparams.epochs + 1):
        # loop over folds
        epoch_acc = 0
        for fold, (train_ids, val_ids) in enumerate(kfold.split(tensor_dataset)):
            print(f"Working on fold {fold} train ids: {train_ids} val ids: {val_ids} - sample_id {hparams.dataset.sample_ids[val_ids]}")

            train_X = dataset.X.iloc[train_ids]
            train_y = dataset.y.iloc[train_ids]

            dataframe = pd.concat([train_X, train_y], axis=1)
            synthetic_data = synthesize_data(dataframe, 200, 0.05)
            
            synthetic_y = synthetic_data['FM_AVERAGE']
            synthetic_X = synthetic_data.drop(columns=['FM_AVERAGE'])
            
            train_dataset = TensorDataset(torch.tensor(synthetic_X.values, dtype=torch.float32), torch.tensor(synthetic_y.values, dtype=torch.float32))
            # initialize dataloaders based on folds
            batch_size = hparams.batch_size
            train_loader = DataLoader(train_dataset, batch_size=batch_size)
            val_subset = Subset(tensor_dataset, val_ids)
            val_loader = DataLoader(tensor_dataset, batch_size=len(val_subset), sampler=SubsetRandomSampler(val_ids))
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(tensor_dataset)):
        ...
        train_loader = DataLoader(tensor_dataset, batch_size=batch_size, sampler=SubsetRandomSampler(train_ids))
        val_subset = Subset(tensor_dataset, val_ids)
